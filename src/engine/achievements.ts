...
export function checkAchievementsAfterMatch(state: SaveState, chaosCardCount: number): string[] {
  ...
  if (last.ourGoals > last.theirGoals) award('first-win')
  if (seasonGoals > 0) award('first-goal')
  ...
  const last5League = leagueResults.slice(-5)
  if (last5League.length === 5 && last5League.every(r => r.rating >= 7)) award('consistent')

  if (last.ourGoals - last.theirGoals >= 3) award('big-win')
  if (last5League.length === 5 && last5League.every(r => r.ourGoals >= r.theirGoals)) award('unbeaten-run')
